...
| `error.subtype` = `permission_denied` | **妙记资源无编辑权**：向妙记所有者申请该妙记的编辑/协作权限；这不是 scope 问题 |
| 缺少 OAuth scope（`error.missing_scopes` 含 `minutes:minutes:update`） | 由 agent 平台为当前用户补开 `minutes:minutes:update` scope |
