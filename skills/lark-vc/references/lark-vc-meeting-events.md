...
查询一场正在进行的视频会议中的会中事件列表。该命令是**读操作**，以 `--as user` 身份运行，读取当前登录用户所在会议的事件。对已结束会议，存在一个**结束后 5 分钟内的宽限窗口**。
...
lark-cli vc +meeting-events --as user --meeting-id <id> --start 2026-04-17T15:00:00+08:00 --end 2026-04-17T16:00:00+08:00 --page-all --format pretty

# 基于上一次保存的 page_token 继续查新增事件
lark-cli vc +meeting-events --as user --meeting-id <id> --page-token <last_page_token> --page-all --format pretty
```
...
如果用户提供的是 9 位会议号，先用 `+meeting-list-active --as user` 查 active meetings 并按 `meeting_no` 匹配。匹配到唯一项后，取该项的长数字 `meeting_id`，再调用本命令。

### 2. 身份是读取事件的权限锚点

- `+meeting-events` 以 `--as user` 身份运行。
- 用户身份发现的会议继续用用户身份读取；不要拿任意 `meeting_id` 直接查。
- 当前用户不是会议可见参与者时，常见结果是空列表或 `no permission`。

### 3. 读取事件前必须先拿到可见的 meeting_id

先通过 active meeting 找到当前登录用户所在会议，再读取事件：
...
- **默认命令模板**：`lark-cli vc +meeting-events --as user --meeting-id <id> --page-all --format pretty`
- 如果你发现自己执行成了不带 `--page-all` 的单页查询，而响应里又出现 `has_more=true` / `more available` / 非空 `page_token`，应立刻意识到这只是部分结果。
- 遇到上述情况，默认补救方式是继续使用返回的 `page_token` 续拉，例如：`lark-cli vc +meeting-events --as user --meeting-id <id> --page-token <returned_page_token> --page-all --format pretty`
...
- 如果上下文没有明确 `meeting_id`，先用 `lark-cli vc +meeting-list-active --as user --format json` 发现当前用户所在会议。返回多个会议时先让用户选择。
- 如果上下文只有 9 位会议号，先执行 `+meeting-list-active --as user` 并按 `meeting_no` 匹配；匹配到唯一会议后再查事件。
- 确认 `meeting_id` 后，沿用其来源身份执行 `lark-cli vc +meeting-events --as user --meeting-id <id> --page-all --format pretty` 拉取最新事件流。
...
| --- | --- | --- | --- |
| `open` | `image` | `element_token` 非空 | `lark-cli docs +media-preview --as user --token "<element_token>" --output "<explicit-path>"` |
| `open` | `whiteboard` | `element_token` 非空 | `lark-cli docs +media-download --as user --type whiteboard --token "<element_token>" --output "<explicit-path>"` |
...
| 输入参数 | 获取方式 |
|---------|---------|
| `meeting-id` | `+meeting-list-active` 返回的 `meeting_id`；或 `+search` 结果中的 `id` |
| `start` / `end` | 用户给出的时间范围；如未给出则默认取全量可见事件 |
| `page-token` | 上一页或上一次查询结果中保存的 `page_token`；建议持久化保存，便于下次继续拉取新增事件 |

## Agent 组合场景

### 场景 1：发现当前用户所在会议并读取会中发生了什么

```bash
# 第 1 步：发现当前登录用户正在参加的会议，拿到 meeting_id
lark-cli vc +meeting-list-active --as user --format json

# 第 2 步：用 meeting_id 读取当前可见事件
lark-cli vc +meeting-events --as user --meeting-id <id> --page-all --format pretty
```
...
| 无权限 / 不可见 | 当前用户不是该会议的可见参与者，或 `meeting_id` 不是从用户身份路径获得 | 确认 `meeting_id` 来自 `+meeting-list-active --as user`；若凭证缺失由 agent 平台补齐用户凭证后重试。**如果只是想看参会人快照，改用 `lark-cli vc meeting get --params '{"meeting_id":"<meeting.id>"}' --with-participants`** |
