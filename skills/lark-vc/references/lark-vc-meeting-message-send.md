...
本命令以 `--as user` 身份运行；`meeting_id` 用 `+meeting-list-active --as user` 发现，发送消息时同样用 `--as user`。
...
```bash
lark-cli vc +meeting-message-send --as user --meeting-id <meeting_id> --msg-type reaction --emoji-type LOVE
lark-cli vc +meeting-message-send --as user --meeting-id <meeting_id> --msg-type reaction --emoji-type VC_NoSound
```
...
4. 以 `--as user` 身份执行 `+meeting-message-send`。
...
权限错误时，不要反复重试；根据错误响应引导用户解决，或让 agent 平台为当前用户补开相应 scope。
