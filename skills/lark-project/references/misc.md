...
| --project-key | string | 是 | 空间 key |
| --work-item-id | string | 是 | 工作项 ID |
| --relation-field-key | string | 否 | 关联关系字段 key，从 `meegle-cli relation meta-definitions` 获取 |
| --relation-id | string | 否 | 关联关系 ID，从 `meegle-cli relation meta-definitions` 获取 |
...
查看评论列表。添加评论用 `meegle-cli comment add`（见 SKILL.md 主文件）。
...
### meegle-cli workhour list-records
查看工作项的工时登记记录。团队排期用 `meegle-cli workhour list-schedule`（见 SKILL.md 主文件）。
...
|------|------|------|------|
| --project-key | string | 是 | 空间 key |
| --work-item-ids | string[] | 否 | 工作项 ID 列表；URL 自动解析；提供名称需先调 `meegle-cli workitem get` 拿 ID |
...
在已启用资源库的工作项类型下创建资源模板（资源实例）。先调 `meegle-cli resource meta-fields` 取字段 / 角色配置。

**创建资源实例 vs 从资源实例创建普通工作项**：
- 创建资源实例：使用 `meegle-cli resource create`；`work_item_type_key` 表示资源库启用的工作项类型，`template_id` 表示流程模板，`fields` / `roles` 描述新资源实例自身。
...
### meegle-cli wbs create-draft
为指定工作项实例创建新的计划表草稿。当需要编辑计划表但当前不存在草稿时，先调本工具创建草稿，再配合 `meegle-cli wbs edit-draft` 进行编辑。
...
### meegle-cli wbs get-draft-progress
...
### meegle-cli wbs list-element-templates
