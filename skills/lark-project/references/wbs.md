...
- **草稿（draft）** — 当前用户的可编辑副本。`meegle-cli wbs create-draft` 创建、`meegle-cli wbs edit-draft` 原子编辑（单行或批量操作）、`meegle-cli wbs publish-draft` 发布、`meegle-cli wbs reset-draft` 放弃修改。
- **实例（instance）** — 已发布的线上版本。只读，用 `meegle-cli wbs list-instance-rows` 查询。

**常见流程**：`meegle-cli wbs create-draft` → 多次 `meegle-cli wbs edit-draft` → `meegle-cli wbs publish-draft`。
...
`meegle-cli wbs list-draft-rows` 与 `meegle-cli wbs list-instance-rows` 参数完全一致，仅查询的数据集不同——前者查当前用户的草稿，后者查线上实例。**禁止混用**：例如先用 `meegle-cli wbs list-draft-rows` 取行 uuid，后续递归 / 编辑也必须继续用草稿系列命令。
...
| `wbs_owner_in_charge` | 负责人 / 责任人（可多人） | userkey；查"我负责的"先调 `meegle-cli user search` 取 userkey |
...
**查工作项字段 / 节点字段**：先用 `meegle-cli workitem meta-fields` 判断是否为工作项字段、用 `meegle-cli workflow meta-node-fields` 判断是否为节点字段；再从计划表行中取对应 `workitem_id` / `state_key`，基于这些 ID 继续查字段值。

### meegle-cli wbs list-draft-rows

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| --work-item-id | string | 是 | 工作项 ID（**字符串**）；URL 自动解析；名称需先调 `meegle-cli workitem get` |
...
参数与 `meegle-cli wbs list-draft-rows` 完全一致，仅查询数据集不同（线上已发布实例）。
...
- 用户说工作项类型（如"需求"、"项目活动"）时用 `AddSubInstanceRows`，并先调 `meegle-cli workitem meta-types` 确认 `work_item_type_key`。
- 涉及资源节点或资源子实例时，先调 `meegle-cli wbs list-element-templates` 查询模板 / `element_key`；资源节点通常用 `AddNodeRows`，资源子实例用 `AddResourceSubInstanceRows`。
- `UpdateRoleOwners` 与 `UpdateDelivery` 是全量覆盖语义。追加负责人或交付物前，先用 `meegle-cli wbs list-draft-rows` 取当前值并合并历史值；不要只传新增值。
...
```json
{
  ...
  "operation_value": {
    "add_task_rows": {
      "parent_uuid": "<上级行 uuid，来自 meegle-cli wbs list-draft-rows>",
      "pre_uuid": "",
      "items": [
        { "name": "新任务名" }
      ]
...
```json
{
  ...
  "operation_value": {
    "add_node_rows": {
      ...
      "phase": "started",
      "items": [
        { "element_key": "<来自 meegle-cli wbs list-element-templates 的 element_key>" }
      ]
    }
  }
}
...
```json
{
  ...
  "operation_value": {
    "add_sub_instance_rows": [
      {
        "parent_uuid": "<上级行 uuid>",
        "work_item_type_key": "<来自 meegle-cli workitem meta-types 的工作项类型 key>",
        "fields": [
...
调用后响应里若有 `change_uuids`，可直接拿去做下一步 `meegle-cli wbs edit-draft`（如改排期）或 `meegle-cli wbs publish-draft` 的部分发布。

**建议工作流**：
1. `meegle-cli wbs list-draft-rows` 取目标行 / 父行的 `uuid` 与当前字段值
2. 调 `meegle-cli wbs edit-draft` 一次执行一种操作
3. 如调用返回了 `operation_id`，先用 `meegle-cli wbs get-draft-progress` 轮询完成再进行下一次编辑——多个 `meegle-cli wbs edit-draft` 并发或不等异步完成就连发可能丢操作
4. 全部改完用 `meegle-cli wbs publish-draft` 发布
...
`meegle-cli wbs create-draft` / `meegle-cli wbs edit-draft` / `meegle-cli wbs publish-draft` / `meegle-cli wbs reset-draft` 返回 `operation_id` 后，需用 `meegle-cli wbs get-draft-progress` 轮询进度。参数表见 [misc.md](misc.md#wbs-辅助命令)。
