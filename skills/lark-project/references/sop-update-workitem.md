...
> **URL 处理**：用户给了 URL 必须先调 `meegle-cli url decode`。只有 `url_kind == workitem_detail` 才能进入本 SOP；其他 kind 按 [url-kinds.md](url-kinds.md) 拒绝或追问。拿到 `simple_name` 和 `work_item_id` 后，必须再调 `meegle-cli project search` 把 `simple_name` 转为权威 `project_key`（同名空间可能有多个）。**禁止**自己从 URL 截取路径段作参数。`work_item_id` 参数必须是字符串类型。

🚨 **获取工作项类型（极重要）**：后续所有查询（字段配置、角色配置等）都强依赖 `work_item_type`。如果用户没有明确告知类型，**必须先调 `meegle-cli workitem get` 获取该实例的真实 `work_item_type`（返回体中的 `work_item_type.key`）**，绝不能猜测为 story 或 issue。
...
| 调用 | 条件 | 说明 |
|------|------|------|
| `meegle-cli workitem meta-fields(field_query="字段名")` | 涉及字段修改 | 用 `field_query` 模糊搜索或 `field_keys` 精确匹配，**禁止逐页遍历** |
| `meegle-cli workitem meta-roles` | 涉及角色修改 | 获取角色 key |
| `meegle-cli user search` | 涉及人员字段 | 批量转换姓名为 userkey |
...
| `user` | 姓名 → `meegle-cli user search` → 单个 userkey 字符串。多人同名时：用户说"分配给我自己"用 `current_login_user()`，否则**必须确认** |
...
**1. 获取旧值** → 调 `meegle-cli workitem get` 或 `meegle-cli workitem query` 查当前值
...
**3. 覆盖写入** → 通过 `meegle-cli workitem update` 写入合并后的值
...
1. 用 `meegle-cli workitem query` 检索符合条件的工作项
2. 提取 ID 列表
3. 按【获取旧值 → 合并 → 写入】流程追加
...
| 场景 | 处理 |
|------|------|
| **节点级字段**（排期、估分、节点负责人） | 不属于本 Skill，需用 `meegle-cli workflow update-node`。如检测到用户要改节点字段，自动调 `meegle-cli workflow update-node` |
...
> **复合明细表已支持通过 `meegle-cli workitem update` 写入**，但两类协议不同：`compound_field` 使用 stringified action 对象并以 `group_uuid` 定位行；`multi_user_compound_field` 使用 stringified userkey map，且是整体覆盖。更新多人复合字段前必须先读当前值并保留全部人员和非目标数据；该协议只更新当前 map 中已有人员，空值或目标人员不在 map 时不能自动新增人员，必须停止并请用户先通过页面配置人员范围。格式详见主文档 [SKILL.md](../SKILL.md)「字段值格式 → 复合明细表」章节。
...
1. **工作项类型未找到**：`meegle-cli workitem meta-types` 失败超过 3 次
...
| 字段名匹配不到 | `meegle-cli workitem meta-fields(field_query="关键词")` 模糊查询 |
| 枚举值匹配不到 | 展示所有枚举值让用户选 |
| 人名匹配到多人 | 展示列表让用户指定，**禁止自行选择** |
| 用户要"追加"而非覆盖 | 走增量追加 SOP |
| 用户要改节点字段 | 自动切换到 `meegle-cli workflow update-node` |
