...
| 现象 | 排查/修复 |
|------|---------|
| 找不到空间 / 中文名匹配多个空间 | `meegle-cli project search` 验证，取 project_key 精确调用 |
| 找不到工作项类型 | `meegle-cli workitem meta-types` 确认合法 type_key |
| 字段名错误 / MQL 返回为空但数据存在 | `meegle-cli workitem meta-fields` 确认字段 key 与类型 |
...
| 人名→userkey 失败 | `meegle-cli user search` 批量查询 |
| 人员字段写入失败 | user 传单个 userkey 字符串；multi-user 必须 stringified 如 `"[\"k1\",\"k2\"]"` |
| node not found | 先 `meegle-cli workitem get` 获取真实 node_id，禁止猜测 |
| 节点流转失败 | 节点流用 `meegle-cli workflow transition`；状态流用 `meegle-cli workflow transition-state`（先 `meegle-cli workflow list-state-transitions` 取 transition_id，再 `meegle-cli workflow list-state-required` 查必填项） |
| 创建工作项缺少模板 | `meegle-cli workitem meta-fields(field_keys=["template"])` 获取 |
| 角色更新失败 | 改用 `meegle-cli workitem update` 的 `role_operate` 参数（不走 fields） |
