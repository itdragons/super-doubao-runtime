...
1. **获取关联字段的目标约束**：从 `meegle-cli workitem meta-fields` 返回的该字段配置中，提取其绑定的**目标空间**（`project_key`）和**目标工作项类型**（`work_item_type_key`）。若配置未限定（可关联任意类型），默认在当前空间内搜索。
2. **按名称搜索目标工作项**：调用 `meegle-cli workitem query`，在目标空间和类型范围内按名称匹配。示例 MQL：
   ```sql
