...
管理应用的文件存储：上传 / 下载本地文件、列出与查看已存文件、生成临时访问链接、批量删除、查看用量。运行时命令事实以 `lark-cli apps +<cmd> --help` 为准。

## 何时用

用户要在某个应用里上传 / 下载 / 列出 / 删除文件、拿文件的临时访问链接、或看存储用量时。普通飞书云盘走 `lark-drive`；数据库里的表数据走 `+db-*`。
