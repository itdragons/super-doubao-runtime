...
用于跟进异步发布任务的状态。只有 `+release-create` 输出 `sync=false` 或缺失时才需要调用本命令。`sync=true` 时发布已同步完成，无需轮询。没有发布标识时先读 [`lark-apps-release-list.md`](lark-apps-release-list.md)，不要让用户手填。

`release_id` 是发布 ID（`+release-create` 返回），不是飞书审批实例号；查发布进度/失败都在 `apps +release-*` 命令族内完成，不要路由到 lark-approval。

## 命令骨架

- 必填：`--app-id`、`--release-id`。
- 发布标识来自 `+release-create` 或 `+release-list`；具体字段名以当前命令输出为准。
