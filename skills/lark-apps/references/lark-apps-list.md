...
列出当前用户的应用，用于从应用名定位 `app_id`。运行时命令事实以 `lark-cli apps +list --help` 为准。
...
- `--app-type` 可按当前 `lark-cli apps +list --help` 支持的枚举过滤；HTML 应用通常按应用名或应用标识定位，不需要主动按类型枚举。
...
lark-cli apps +list --ownership mine
...
多候选时展示名称、app_id、updated_at 让用户确认。用户描述里已经有 `app_xxx` 或应用链接时，直接提取，不再 `+list`。

把 `+list` 当定位工具和发布态快照工具，不要把 `is_published` 当部署完成证明。需要证明“最新内容已上线”时，使用本轮发布任务的 `+release-get` 完成状态。
