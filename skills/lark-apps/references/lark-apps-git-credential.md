...
应用 Git 凭证用于本地原生 `git clone/pull/push`。运行时命令事实以 `lark-cli apps +git-credential-init --help`、`+git-credential-list --help`、`+git-credential-remove --help` 为准。
...
- `+git-credential-list` 列出本地已配置的 Git 凭证，不需要 `--app-id`。
