...
对指定 slide 做块级替换或插入。编辑已有幻灯片的主路径——`slide_id` 不变、页序不动、只影响被指定的块。
...
cat parts.json | lark-cli slides +replace-slide \
...
`<polyline>`：
```xml
<polyline topLeftX="10" topLeftY="10" width="100" height="50">
...
```bash
# 读时记录 revision_id
REV=$(lark-cli slides xml_presentation.slide get \
  --params "{\"xml_presentation_id\":\"$PID\",\"slide_id\":\"$SID\"}" \
  | jq '.data.revision_id')
...
- [slides-editing.md](../workflow/slides-editing.md) — 编辑流程与操作选型
