...
2. 用 `slides +xml-get --output <CWD 内相对路径>` 回读全文 XML 到本地文件（`--output` 必填；必须先用 XML 解析器解析，命名空间从根元素实际读取，不要硬编码否则匹配不到元素），并以当前结果建立本次校验的 `slide_ids` 页清单。首次新建且页集合未变时，可复用创建响应；增删页、整页替换或重排后必须刷新清单。
