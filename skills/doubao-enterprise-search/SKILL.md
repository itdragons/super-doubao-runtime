...
description: 当用户显式使用 /doubao-enterprise-search，或问题明确/高度可能依赖企业内部资料、内部口径、内部系统信息、业务背景、历史讨论、协作记录、制度流程、申请方法、员工权益、差旅报销、内部产品/业务系统/第三方平台语境中的非飞书通讯录 ID/UID/账号/标识时，进入本 skill 判断是否调用 `enterprise_agentic_search` 工具。内容总结、讨论复盘、会议/群聊/项目进展整理、结论/待办/风险/反馈提炼，或会议/群聊/文档/日程/链接只是检索线索时，也应进入本 skill 判断；不要仅因办公写作、通用职场建议、普通内容生成或明确只查公开信息而进入。
---

# doubao-enterprise-search（企业搜索）

## 必读摘要（先读这里）
**必须**全文通读本 skill；摘要只用于防止遗漏关键规则，不能替代正文。

1. `enterprise_agentic_search` 是企业内部知识问答 / 推理整合工具，不是关键词搜索引擎；query 必须是完整自然语言问题。详见第 2 章。
...
5. 使用 `enterprise_agentic_search` 工具结果时，必须保留引用、来源链接、材料入口链接；Markdown 材料链接要原样迁移。详见第 4 章。
...
2. 同一用户问题默认只调用一次；多个内部资料子问题应合并到一次调用中。
3. 仅在以下情况应该再次调用：
...
**query 要求**
1. 使用完整、可独立理解的自然语言问题或陈述句，不得写成搜索引擎关键词。
...
- 优先保留其中能直接回答用户问题的信息。
- 可以压缩、重排和合并，但不能无依据地改写结论、扩大范围，或把不确定内容说成确定事实。
...
### 4.3 引用与材料链接要求
使用 `enterprise_agentic_search` 工具结果时，必须同时处理好“事实来源引用”和“材料入口链接”。

#### 事实来源引用

- **工具返回引用必须保留**：只要最终回答使用、改写、总结或转述了 `enterprise_agentic_search` 工具返回的事实、数字、观点、结论、流程、制度、规则、申请方法、入口路径或原文，就必须同步迁移对应引用，不得丢弃原答案中的引用标签或来源链接。
- **无法对应引用就不要输出该事实**：如果无法确定某条事实对应的来源引用，不要输出这条来自 `enterprise_agentic_search` 工具的事实；可以改为说明“未找到可引用的内部资料”或只保留有来源的内容。
- **引用要贴近对应内容**：对结果进行压缩、合并、分点整理或表格化时，必须先确认每条事实由哪些材料及其 URL 支撑，并在写出该事实时同步添加对应引用。引用必须跟随对应事实放在同一句、同一列表项或同一表格单元格中；**禁止**先输出没有引用的完整正文，再将文档链接或引用集中放到回答末尾。
- **多来源引用合并**：同一事实由多个来源支撑时，必须合并为一个 `<RichMediaReference>["url1","url2","url3"]</RichMediaReference>`标签。
...
- **表格引用不得换行**：整理成表格时，引用必须紧跟在同一单元格的被引用内容后，不得在单元格内换行后单独放置引用，也不得在表格结束后统一补引用。
- **组织性语言通常不需要引用**：模型自己的标题、过渡句、总结框架、操作说明等不需要引用；但如果这些句子中包含来自 `enterprise_agentic_search` 工具的具体事实，该事实部分仍必须带引用。

#### 材料入口链接与图片

- **材料入口链接应保留但不要重复引用**：材料入口链接必须保留，不要只输出摘要；若材料以 Markdown 链接返回（如 `[材料标题](https://example.com/...)`），最终回答也必须保留完整 Markdown 链接，不得只输出标题。单独展示的材料标题、材料入口或“相关材料”列表只是方便用户打开材料，不属于对材料事实的转述；不得在 Markdown 链接后追加相同 URL 的 `<RichMediaReference>`，也不得输出 `[材料标题](URL)<RichMediaReference>["URL"]</RichMediaReference>` 这种重复格式。如果正文使用、改写、总结或转述了该材料中的具体事实，仍须在对应事实后就地添加引用，不能用材料入口链接代替事实引用。
- **图片与图注应保留**：若 `enterprise_agentic_search` 返回图片资源，模型应根据用户问题和答案需要决定是否使用图片；当图片承载关键信息，或能显著帮助理解步骤、结构、对比、流程、界面位置、数据关系时，应在最终回答中保留该图片和图注。不要机械输出所有图片；优先保留与答案直接相关、信息量最高的图片。若最终回答以“序号 / 列表项 + 图片图注 + Markdown 图片”的形式展示，`<RichMediaReference>` 必须紧跟在图注文本后、Markdown 图片语法前；不要把引用放在 `![图片](图片URL)` 后面，也不要在图片下方重复输出图注。格式为：`1. {图注}<RichMediaReference>["{来源URL}"]</RichMediaReference>` 换行后 `   ![{图注}]({图片URL})`。`{来源URL}` 的选择规则：若工具返回 `<search><document ...>` 片段集合，使用图片所属或相邻 `<document>` 的 `<url>`；若工具返回 `<answer>` 参考答案，使用图片前最近的可用来源 URL / 引用链接。
...
- **公司 / 办公语境下的生活服务**：内部食堂、园区福利、公司班车、内部活动、员工专属渠道等依赖内部资料的问题 → 调用 `enterprise_agentic_search` 工具；公司楼下有什么好吃的、附近商圈、公开餐饮推荐、外部商业信息等不依赖内部资料的问题 → 公开搜索或直接回答。
