...
本 skill 所有命令统一以 `--as user` 身份运行。知识空间和节点都是用户的个人资源，命令中显式带上 `--as user`。
...
- 用户给的是知识库 URL（`.../wiki/<token>`），且后续要查成员/加成员/删成员：调用 `lark-cli wiki +node-get --node-token '<wiki_url>' --as user --format json`，从 `data.space_id` 获取空间 ID。
- 用户要**删除**知识空间（`wiki +delete-space`）但只给了名称或 URL：**不能**把名称 / URL 原样传给 `--space-id`，必须先解析出真实 `space_id`。解析方式：
  - URL（`.../wiki/<token>`）：调用 `lark-cli wiki +node-get --node-token '<wiki_url>' --as user --format json`，读取 `data.space_id`。
...
- 用户说“用户 / 群 / 部门 / 应用 + 添加成员”：先解析对应 ID，再执行 `wiki +member-add --as user`。
