...
豆包文档、豆包表格和豆包 PPT 在当前运行时分别对应飞书在线文档、表格和 PPT。本文件定义用户选择豆包/飞书在线载体，或未指定载体时，QA 内容如何映射。实际创建、编辑、回读和权限处理交给 `lark-doc`、`lark-sheets`、`lark-base`、`lark-ppt`，不在本 Skill 重写其内部命令。
