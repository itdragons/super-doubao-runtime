...
- 用户要查看、下载、回滚或删除文件的**历史版本**，使用 `drive +version-history`、`drive +version-get`、`drive +version-revert`、`drive +version-delete`。
...
知识库链接背后可能是 docx、sheet、bitable、slides、file 等不同对象。后续要做评论、下载、导出或内容读取时，优先用 `drive +inspect` 拿到 `type`、`token`、`title`、`url`。不要只根据 `/wiki/<token>` 猜底层类型。
...
- 普通 scope、身份或登录问题，根据错误响应引导用户解决；不要把租户安全策略、对外分享、密级拦截简单归类为缺 scope。
